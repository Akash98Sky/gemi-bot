...
from chat.services.vector_db import VectorDbService
...
class BotContainer(containers.DeclarativeContainer):
    ...
    vector_db_service = providers.Singleton(VectorDbService, url=Configs.chat_config.qdrant_db_url, api_key=Configs.chat_config.qdrant_api_key)
    query_processor = providers.Singleton(QueryProcessor, gemini=chat_service, voice=voice_service, img_gen=img_service, vector_db=vector_db_service)
    prompt_generator = providers.Singleton(PromptGenerator, voice_service=voice_service)
    chat_repo = providers.Factory(ChatRepo, gemini=chat_service, processor=query_processor, generator=prompt_generator)
